...
pkgname=minecraft-server-latest
pkgver=$(curl -s 'https://launchermeta.mojang.com/mc/game/version_manifest.json' | jq -r '.versions[0].id')
sourceUrl=$(curl "$(curl -s 'https://launchermeta.mojang.com/mc/game/version_manifest.json' | jq -r '.versions[0].url')" | jq -r '.downloads.server.url')
pkgrel=1
